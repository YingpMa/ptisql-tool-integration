...
class SimpleVNetwork(SoftQNetwork):
    ...
    def _forward(self, x, *args):
        ...
        x = self.fc3(x)
        return x
class SimpleQ2Network(SoftQNetwork):
    def __init__(self, obs_dim, action_dim, args, device='cpu'):
        super(SimpleQ2Network, self).__init__(obs_dim, action_dim, args, device)
        self.args = args
        #self.layers = nn.ModuleList([nn.Linear(obs_dim, 264)])
        #self.out = nn.Linear(264, action_dim)
        self.fc1 = nn.Linear(obs_dim, 264)
        self.fc2 = nn.Linear(264, action_dim)
