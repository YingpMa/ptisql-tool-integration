...
def evaluate(actor, env, seed_eval, num_episodes=10, vis=True):
    ...
    while len(total_returns) < num_episodes:
        state, _ = env.reset(seed=seed_eval)
        done = False
        ...
        with eval_mode(actor):
            while not done and not env_step_limit_reached:
                action = int(actor.choose_action(state, sample=False))
                next_state, reward, done, env_step_limit_reached, info = env.step(action)
                if reward < -90:
                    h = 1
                state = next_state
                r += reward
