...
def main(cfg: DictConfig):
    ...
    projectname=args.env.name.split(':')[-1]
    wandb.init(project=projectname, entity='lechitu',
#               sync_tensorboard=True, reinit=True, config=args)
    ...
    eval_env = make_env(args, monitor=False)
    
    if 'nasim' not in args.env.name:
...
#        eval_env.reset(seed=args.seed + 10)
    ...
    state_0 = [env.reset(seed=args.seed)[0]] * INITIAL_STATES
    ...
    for epoch in count():
        ...
        for episode_step in range(EPISODE_STEPS):
            ...
            if steps < args.num_seed_steps:
                ...
                with eval_mode(agent):
                    action = int(agent.choose_action(state, sample=True))
            next_state, reward, done, env_step_limit_reached, _ = env.step(action)
            if reward < -90:
                honeypot = 1
            ...
            if learn_steps % args.env.eval_interval == 0:
                eval_returns, eval_timesteps,goal_reached,honeypotlist = evaluate(agent, eval_env, seed_eval=args.seed + 10, num_episodes=args.eval.eps)
                returns = np.mean(eval_returns)
                reached = np.mean(goal_reached)
                honey = np.mean(honeypotlist)
                learn_steps += 1  # To prevent repeated eval at timestep 0
                logger.log('eval/episode_reward', returns, learn_steps)
                logger.log('eval/episode', epoch, learn_steps)
                logger.log('eval/honeypot',honey, learn_steps)
                logger.log('eval/goal_reached',reached, learn_steps)
        ...
        logger.log('train/goal_reached', goal_reached, learn_steps)
        logger.log('train/honeypot',honeypot,learn_steps)
